-- GovContractAI Final Database Setup (FIXED)
...
SELECT 
  ...
  'active',
  1000,
  FLOOR(RANDOM() * 100 + 10)::INTEGER  -- Random usage between 10-110 for realistic data
FROM companies 
WHERE NOT EXISTS (
...
-- 12. Enable RLS (Row Level Security) on tables
ALTER TABLE applications ENABLE ROW LEVEL SECURITY;
ALTER TABLE subscriptions ENABLE ROW LEVEL SECURITY;
ALTER TABLE certifications ENABLE ROW LEVEL SECURITY;
ALTER TABLE user_course_progress ENABLE ROW LEVEL SECURITY;

-- 13. Drop existing policies if they exist (to avoid conflicts)
...
-- 14. Create RLS policies (without IF NOT EXISTS)
